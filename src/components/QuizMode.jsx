...
function Stars({ score, total }) {
  ...
  return (
    <div className="flex gap-2 justify-center text-4xl" aria-label={`${stars} out of 3 stars`}>
      {[1, 2, 3].map((s) => (
        <motion.span
          key={s}
...
function GraphToEquation({ question, onAnswer, answered }) {
  return (
    <div className="flex flex-col gap-4">
      ...
      >
        <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">What is the equation of this line?</p>
  ...
  return (
    <div className="flex flex-col gap-4">
      <div
        ...
        style={{ background: '#1E293B', border: '1px solid #334155' }}
      >
        <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Adjust the line to match this equation:</p>
      ...
      >
        <CoordinatePlane
          ...
          highlightLine
          lineColor={
            answered === null ? '#2DD4BF' :
            answered === 'correct' ? '#22c55e' : '#ef4444'
          }
        />
      </div>
      ...
      <div
        ...
        style={{ background: '#1E293B', border: '1px solid #334155' }}
      >
        <Slider label="Slope (m)" value={userM} min={-5} max={5} step={0.5}
          onChange={setUserM} color="#2DD4BF" ariaLabel="Adjust slope" />
        <Slider label="Y-intercept (b)" value={userB} min={-10} max={10} step={1}
          onChange={setUserB} color="#FCD34D" ariaLabel="Adjust y-intercept" />
      </div>
        ...
        >
          {answered === 'correct' ? (
            <>
              ...
              <p className="font-extrabold text-green-300 mt-1">Correct!</p>
            </>
          ) : (
            <>
              <span className="text-2xl">💡</span>
              <p className="font-extrabold text-red-300 mt-1">Not quite!</p>
              <p className="text-red-200 text-sm mt-1">
                Answer: m = {question.m}, b = {question.b}
              </p>
            </>
          )}
        </div>
...
export default function QuizMode() {
  ...
  const score = Object.values(answers).filter((a) => a === question.correct || a === 'correct').length;

  // Actually track per-question correctness properly
  const correctCount = QUIZ_QUESTIONS.filter((q) => {
    const a = answers[q.id];
    if (a === undefined) return false;
    if (q.type === 'graphToEquation') return a === q.correct;
    return a === 'correct';
  }).length;
  ...
  if (showEnd) {
    return (
      ...
      >
        ...
        <h2 className="text-3xl font-black text-white text-center">Case Closed!</h2>
        <div
        ...
        >
          ...
          <p className="text-slate-400 text-sm mb-4">
            {correctCount >= 7 ? 'Brilliant detective work! 🌟' :
             correctCount >= 5 ? 'Great job! Keep practising! 💪' :
             'Keep going — you\'ll crack it! 🔍'}
          </p>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h2 className="text-2xl font-extrabold text-white">🕵️ Line Detective</h2>
          <p className="text-slate-400 text-sm mt-0.5">Quiz time — can you crack the code?</p>
        </div>
        {/* Score tracker */}
        <div
          className="flex items-center gap-2 px-4 py-2 rounded-xl"
          style={{ background: '#1E293B', border: '1px solid #334155' }}
          aria-label={`Score: ${correctCount} out of ${QUIZ_QUESTIONS.length}`}
        >
      ...
      <div>
        <div className="flex justify-between text-xs text-slate-500 mb-1">
          ...
          <span>{question.type === 'graphToEquation' ? 'Graph → Equation' : 'Equation → Line'}</span>
        </div>
      ...
      <div className="flex gap-2 justify-center flex-wrap">
        {QUIZ_QUESTIONS.map((q, i) => {
          ...
          return (
            <div
              key={q.id}
              aria-label={`Question ${i + 1}${isDone ? (isCorrect ? ', correct' : ', wrong') : ''}`}
              className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold"
              style={{
                background:
                  i === qIdx ? '#2DD4BF' :
                  isDone && isCorrect ? '#14532d' :
                  isDone ? '#450a0a' :
                  '#1E293B',
                border: `1.5px solid ${
                  i === qIdx ? '#2DD4BF' :
                  isDone && isCorrect ? '#22c55e' :
                  isDone ? '#ef4444' :
                  '#334155'
                }`,
                color:
      ...
      <AnimatePresence>
        {answered !== null && (
          ...
          >
            <button
              onClick={handleNext}
              aria-label={qIdx < QUIZ_QUESTIONS.length - 1 ? 'Next question' : 'See results'}
